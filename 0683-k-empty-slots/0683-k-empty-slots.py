...
class Solution:
    def kEmptySlots(self, bulbs: List[int], k: int) -> int:
        ...
        for day, pos in enumerate(bulbs):
            ft.update(pos, 1)
            if pos - k - 1 > 0 and ft.rangeSum(pos - k - 1, pos - k - 1) == 1 and ft.rangeSum(pos - k, pos - 1) == 0:
                return day + 1
            if pos + k + 1 <= n and ft.rangeSum(pos + k + 1, pos + k + 1) == 1 and ft.rangeSum(pos + 1, pos + k) == 0:
                return day + 1
        return -1